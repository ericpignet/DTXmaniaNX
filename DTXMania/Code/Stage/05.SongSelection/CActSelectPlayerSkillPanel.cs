    ...
    internal class CActSelectPlayerSkillPanel : CActivity
    {
        ...
        public override void OnManagedReleaseResources()
        {
            if (!base.bNotActivated)
            {
                ...
                CDTXMania.tReleaseTexture(ref this.txGroupName);
        ...
        public override int OnUpdateAndDraw()
        {
            if (!base.bNotActivated)
            {
                ...
                if (CDTXMania.stageSongSelection.rCurrentBox != null && this.txPlayerSkillPanel != null)
                {
                    ...
                    if (this.txGroupName != null)
                    {
                        this.txGroupName.tDraw2D(CDTXMania.app.Device, nPlayerSkillsPanelPositionX + 4, nPlayerSkillsPanelPositionY + 1);
                    }
                    if (this.txPlayerName != null)
                    {
                        this.txPlayerName.tDraw2D(CDTXMania.app.Device, nPlayerSkillsPanelPositionX + 2, nPlayerSkillsPanelPositionY + 14);
                    }
        ...
        public void tGeneratePlayerSkillPoints()
        {
            if (CDTXMania.stageSongSelection.rCurrentBox != null)
            {
                ...
                if (CDTXMania.stageSongSelection.rCurrentBox.dbBoxSkillPoints > 0)
                {
                    ...
                    CDTXMania.tReleaseTexture(ref this.txGroupName);
                    string strGroupName = string.IsNullOrEmpty(CDTXMania.ConfigIni.strGroupName[instrument]) ? "" : CDTXMania.ConfigIni.strGroupName[instrument];
                    CPrivateFastFont pfGroupName = new CPrivateFastFont(new FontFamily(CDTXMania.ConfigIni.str選曲リストフォント), 14, FontStyle.Regular);
                    Bitmap bmpGroupName;
                    bmpGroupName = pfGroupName.DrawPrivateFont(strGroupName,
                                                               CPrivateFont.DrawMode.Gradation,
                                                               Color.White,
                                                               Color.White,
                                                               Color.White,
                                                               Color.White);
                    this.txGroupName = CDTXMania.tGenerateTexture(bmpGroupName, false);
                    bmpGroupName.Dispose();
                    pfGroupName.Dispose();
